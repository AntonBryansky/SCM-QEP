...
M = get_M_matrix(N, density);      % M matrix does not depend on the wavenumber
M = set_M_BC(M, N);                % Setting boundary conditions for M matrix

% Init output data storage
freqs_output = NaN(length(wavenumber), N*3);
PV_output = NaN(length(wavenumber),N*3);
PV_S_output = NaN(length(wavenumber),N*3);
PV_AS_output = NaN(length(wavenumber),N*3);
k_output =  NaN(length(wavenumber),N*3);
Lamb_output = NaN(length(wavenumber), N*3);
Lamb_S_output = NaN(length(wavenumber), N*3);
Lamb_AS_output = NaN(length(wavenumber), N*3);
SH_output = NaN(length(wavenumber), N*3);
SH_S_output = NaN(length(wavenumber), N*3);
SH_AS_output = NaN(length(wavenumber), N*3);
...
for i = 1:n_wavenumber
    ...
    L = get_L_matrix(c, k, I, DM1, DM2);
    S = get_S_matrix(c, k, I, DM1);
    ...
    for j = 1:3*N
        ...
        if Vp(j) ~= 0 && isfinite(Vp(j)) % Skip values equal to zero or Inf
            ...
            if (abs(U21) <= err_coeff) && (abs(U2n) <= err_coeff) % Pure Lamb wave
                % Case of nulity displacement at the borders of U2 interface
                if (abs(U11 - U1n) <= err_coeff) && (abs(abs(U31) - abs(U3n)) <= err_coeff) && (abs(U1n) >= err_coeff || (abs(U31 + U3n) <= err_coeff))
                    % Symmetric Lamb mode:
                    % Interface U1: displacements on the borders are equal;
                    % Interface U3: modulus of displacements on the borders are equal;
                    % Displacements on the borders of U1 should be greater than zero
                    % or displacements on the borders of U3 should be opposite in sign
                    Lamb_S_output(i, j) = Vp(j);
                else
                    % Otherwise the Lamb mode is antisymmetric
                    Lamb_AS_output(i, j) = Vp(j);
                end
                Lamb_output(i, j) = Vp(j);
            elseif (abs(U11) <= err_coeff) && (abs(U1n) <= err_coeff) && (abs(U31) <= err_coeff) && (abs(U3n) <= err_coeff) % Pure SH mode
                % Case of nulity displacement at the borders of U1 and U3 interfaces
                if (abs(U21 - U2n) <= err_coeff)
                    % Symmetric SH mode
                    SH_S_output(i, j) = Vp(j);
                else
                    % Otherwise the SH mode is antisymmetric
                    SH_AS_output(i, j) = Vp(j);
                end
                SH_output(i, j) = Vp(j);
            end

            % General parity separation for plotting all calculated modes
            if (U1(1, j)*U1(end, j) >= 0) && (U2(1, j)*U2(end, j) >= 0) && (U3(1, j)*U3(end, j) < err_coeff)
                % Symmetric mode
...
figure('Color','w');
plot(freqs_output*1e-3, PV_output, '.k');
ylim([0 PV_limit]);
...
Kmat = repmat(wavenumber(:), 1, 3*N);   % each row has the same input k
...
figure('Color','w');
plot(freqs_output*1e-3, Kmat,  '.k', 'MarkerSize', 4);
xlim([0, freq_limit*1e-3]);
ylim([0, WN_limit]);
xlabel('Frequency (kHz)');
